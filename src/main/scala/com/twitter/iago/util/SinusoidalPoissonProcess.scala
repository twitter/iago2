 ...
 * a sine wave pattern, rising to maxRate in 1/4 period and returning to maxRate every period thereafter.
 *
...
class SinusoidalPoissonProcess(
  ...
  if (minRate > maxRate) {
    throw new IllegalArgumentException(
      "min arrival rate (%d) must be less than max (%d)".format(minRate, maxRate)
    )
  }

  if (minRate <= 0 || maxRate <= 0) {
    throw new IllegalArgumentException(
      "min (%d) and max (%d) arrival rates must be greater than 0".format(minRate, maxRate)
    )
  }
