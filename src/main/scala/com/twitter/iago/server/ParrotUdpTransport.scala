...
abstract class ParrotUdpTransport[Rep](
  victim: InetSocketAddress,
  statsReceiver: StatsReceiver = DefaultStatsReceiver
) extends ParrotTransport[ParrotRequest, Rep] {

  private[this] val scopedStats = statsReceiver.scope("udp_transport")
  private[this] val udpRequestTimeout = scopedStats.counter("udp_request_timeout")
  private[this] val udpRequestLatencyUsec = scopedStats.stat("udp_request_latency_usec")
  ...
  override def shutdown() = {
    FuturePool.immediatePool {
      channelFutureMap.values.asScala.foreach {
        _.raise(new FutureCancelledException)
      }
